...
import { ContextMenuProvider } from '@shared/ui/ContextMenu';
import './Map.scss';
import { useMap } from './UseMap';

const Map = () => {
 const { points, loading, changePoint, setPoints, deletePoint, addPoint, resetPoints } =
  usePointStore();
  ...
  <div className="map flex gap-4 h-100">
   <div className="map__container h-100" onMouseMove={handleMouseMove}>
    {loading ? (
     ...
     <ContextMenuProvider
      contextMenuNode={cords => <Button onClick={() => addPoint(cords)}>Создать</Button>}
     >
      <div
